...
// invalid_credentials라 /auth/email-exists 프로브로 구분한다. 소셜은 카카오 1개(R-10 — 시안의 네이버·구글은
// 미구현 기능이라 미배치), PKCE 콜백(/auth/callback)에서 세션 교환 후 복귀. 보호자 화면 — 스크롤 허용.
...
const inputClass =
  'h-[50px] w-full rounded-[14px] border border-[#F0E4D3] bg-base px-4 text-base text-ink outline-none placeholder:text-ink/50 focus:border-primary';
...
export function LoginScreen({ initialSocialError }: { initialSocialError: boolean }) {
  ...
  return (
    <main className="flex min-h-dvh flex-col items-center bg-[#F2EFE8] px-6 py-9">
      ...
      <section className="mt-7 w-full max-w-[480px] rounded-3xl bg-base p-8 shadow-[0_4px_32px_rgba(58,44,30,0.10)]">
