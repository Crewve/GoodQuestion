...
export function buildGenerateMessages(context: GenerateContext) {
  ...
  // '친구야'는 그 자체가 호격 — 이름처럼 '아/야'를 덧붙이면 "친구야아/야"로 깨진다 (C1 보조)
  const callExample = context.childName ? `'${childCall}아/야'처럼 호격으로` : `'${childCall}'라고`;
  // 주어 예시는 말하는 캐릭터가 아닌 인물로 고른다 — 자기 자신이 예시면 모델이
  // "며느리는 왜 참았을까?"를 그대로 복사해 3인칭 자기 지칭이 된다 (simulate 회귀에서 확인)
  const subjectExample = character.name === '며느리' ? '시아버지는 왜 화가 났을까?' : '며느리는 왜 참았을까?';
...
[역할 유지 — 반드시 지킨다]
- 너는 '${character.display_name}' 본인이다 — 자신을 '${character.name}'라고 3인칭으로 부르지 않는다. 네 이야기는 '나'로 말하고, 네 마음을 아이에게 물을 때도 "내가 어떤 기분이었을 것 같아?"처럼 '나'로 묻는다
...
- 다른 인물의 사정·마음을 네가 설명하거나 두둔하지 않는다 — 아이가 네 편을 들어도 다른 인물을 대신 변호하지 말고, 그 인물의 마음은 질문으로만 꺼낸다 (예: "그 사람 마음은 어땠을까?")
...
- 아이 이름은 '${childCall}' — 부를 때는 ${callExample} 자연스럽게 부른다 (성을 붙이거나 이름 뒤에 쉼표를 찍지 않는다)
