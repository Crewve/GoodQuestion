# 코드 계약: `src/lib/contracts.ts` (파트1 ↔ 파트2)

두 파트의 접점은 텍스트 2곳뿐이라는 분담 원칙을 타입으로 고정한다. **이 파일 변경은 상호 합의로만** (분담안 공용 규칙). 초안은 분담안 §모듈 인터페이스를 디렉토리 명세 경로(`@/lib/contracts`)로 확정한 것.
...
1. `failed: true`인 `SttResult`는 어떤 경로로도 `messages` 행이 되지 않는다.
2. `AnalysisResult.detectedElements[].evidence`는 아이 발화 원문의 부분 문자열이어야 하며, 위반 시 서버 후처리가 해당 탐지를 제거·보정한다 (LLM 원본은 raw로 보존).
3. `ProgressDecision`은 `story_sessions` 상태(accumulated/카운터)와 장면 데이터(required_elements·max_turns·미션 정의)만으로 결정된다 — LLM 출력이 판단을 직접 결정하지 않는다.
