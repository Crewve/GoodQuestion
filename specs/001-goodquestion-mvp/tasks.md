...
- [ ] T068 \[P\] (파트1) 프로필 캐릭터 이미지 매핑 버그 — 프로필 생성 시 선택한 캐릭터와 생성 후 카드에 표시되는 캐릭터가 다름. `child-profile-form.tsx`(선택 저장 avatar_key) ↔ `profiles-screen.tsx`·홈(표시 매핑) 대조해 매핑 불일치 수정 (E2E 항목 5. 기획 위반 버그 — 캐릭터 선택값 그대로 표시가 명세)
