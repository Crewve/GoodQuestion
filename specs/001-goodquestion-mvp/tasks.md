...
- [ ] T075 (파트2) 입력측 부적절 발화 처리 — 비속어·주제/상황과 무관한 발화 시 캐릭터가 평가·지적 없이 부드럽게 주제로 복귀 유도: `analysis.ts` utterance_validity 판정 활용 + `generate.ts` 금칙어 목록 재사용해 유도 응답 분기 (E2E 항목 24. FR-009 utterance_validity·FR-011 아동 안전 가드레일의 입력측 확장 — 기획 무충돌 보완)
