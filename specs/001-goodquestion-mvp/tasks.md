...
- [ ] T073 \[P\] (파트2) 캐릭터 프롬프트 보강 — `src/lib/llm/generate.ts` 2건: ① 역할 고정 — 아이가 상대 캐릭터 편을 들어도 자기 캐릭터의 입장·감정을 유지(시아버지가 며느리 편드는 이탈 방지) ② 미션 직전 브릿지 — 장면 목표(방귀 활용)로 전환 시 아이 발화를 받아 자연스럽게 연결(급전환 방지). 수정 후 `eval/run.ts` 회귀 필수(T028 관례) (E2E 항목 17·22. FR-011 페르소나 반영·기획 "자연스럽게 연결" 요구의 품질 결함)
