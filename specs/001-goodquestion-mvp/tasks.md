...
- [ ] T076 (파트2) 대화 내역 리스트 — `dialogue-scene`에 아이·캐릭터 발화 누적 말풍선 리스트(색상·정렬 구분, 우측 배치) 추가. 화면 스크롤 미허용 원칙과의 조화는 리스트 영역 내부 스크롤(overflow-y)로 처리 — 화면 전체 스크롤 아님 판단 기록 (E2E 항목 15. 기능명세서 2.4.3 "대화 내역 리스트 — 필수" 미구현. 이전 대사 소리 재생은 제외 — 상단 적용 제외 참고)
