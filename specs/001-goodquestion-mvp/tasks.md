...
- [x] T057 \[P\] \[US5\] (파트2) 프로필 관리 `src/app/my/profiles/page.tsx` — 목록+아이 추가(2.1.1 폼 재사용, 3명 제한) ✓ 완료(2026-08-12): 서버 셸(2.1 T047과 동일 쿼리·이중 방어)+`manage-screen.tsx`. 카드 리스트는 표시 전용(아바타·이름·만 나이), 아이 추가는 `ChildProfileForm`·`save-profile.ts` 그대로 재사용(화면 내 뷰 전환). 3.2 요건대로 3명이면 **버튼 비활성+초과 문구 노출**(2.1의 '카드 숨김'과 다른 규칙 — 명세 원문 반영), 빈 상태 문구 3.2 원문. GNB 마이페이지 활성, 뒤로가기 → /my(T056 합류 시 동작). ⚠️ 구성요소 표의 '프로필 관리' 버튼은 동작·이동 미정의(수정/삭제는 MVP 밖)라 미구현 — 기획 확인 필요. 브라우저 실측: 목록→추가 폼→등록(만 나이 배지 재계산)→3명 도달 시 비활성+문구 확인(테스트 아이 2명은 DB 원복)
