...
- [ ] T074 (파트2) 미션 강제 노출 폴백 ★데모 치명 — `src/lib/rules/engine.ts`: 미션 장면(대화3·4)에서 미션 미노출 상태로 ① 2턴 경과 시 강제 노출(미션 노출 기준 문서의 공통 조건 "2회 이상 대화했지만 실행 방법이 나오지 않은 경우"를 코드로 보장) ② CLOSING 판정 전 미션 미수행이면 노출 선행. 테스트 추가 (E2E 항목 21. 미션1·2는 Must이고 종료 조건에 "미션+핵심 발화 완료" 포함 — 미션 없이 장면 종료는 기획 위반 버그)
