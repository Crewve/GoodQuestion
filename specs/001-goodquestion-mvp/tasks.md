...
- [ ] T066 \[P\] (파트2) index 라우팅 재정의 — `src/app/page.tsx`의 create-next-app 보일러플레이트 제거, 서버 컴포넌트에서 세션 확인 후 미인증→`/login`, 인증→`/profiles` 리다이렉트 (E2E 항목 31·9 후반. proxy.ts 보호 프리픽스에 `/`가 없어 루트만 뚫려 있음)
