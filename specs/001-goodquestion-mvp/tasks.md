...
- [x] T060 (공동) 장애 폴백 리허설 — TTS 키 제거 상태 CLOSING 재생, LLM 장애 1회 재시도·고정 대사 폴백, max_turns 직전 타임아웃 강제 종료 (contracts/api-routes.md 폴백 매트릭스 전항) ✓ 완료(2026-08-12): 팀 리허설로 폴백 매트릭스 전항 통과 — TTS 장애 시 텍스트만 표시·CLOSING 사전 생성본 무영향, 분석/생성 LLM 장애 1회 재시도→고정 대사 폴백, Whisper 실패 재시도 문구·메시지 미생성, max_turns 직전 지연 타임아웃 강제 종료. 절차·유발 방법·결과는 `test-checklist.md` §C(원상복구 C-6 포함)

- [x] T064 (공동) quickstart.md 전체 검증 실행 — §1\~§4 E2E 데모 시나리오 리허설, 타입캐스트 사용량·무료 한도 재확인 (T060 이후 마지막) ✓ 완료(2026-08-12): §4 E2E 8단계 리허설 완주(회원가입→대화·미션→최대 턴 클로징→학습완료 활동→재개→출처 표기) + §5 재확인 — E-1 버킷 3종 전수(사용자 녹음 0건, tts-cache 증가분은 합성 캐시), E-2 번들 키 실값·이름 0건+양성 대조군 통과, E-3 테스트 계정 2종 캐스케이드 정리(잔존 0·시드 유지). 타입캐스트 잔여 한도 데모 가능 팀 확인. 전 기록: `test-checklist.md`(전 항목 [x], 발견 이슈 0건). **MVP 태스크 전체 완료**

---

## Phase 9: E2E 피드백 수정 (2026-08-12 팀 E2E 테스트)

**Goal**: 팀 E2E 테스트 수정사항 리스트(단위테스트 후 수정사항리스트.txt) 15건 중 기획 부합 9건 + 기획 무충돌 보완 4건을 반영한다. 아래 태스크는 **구현·수정하기 쉬운 순** 정렬 — 단 T074(미션 미노출)는 데모 성립에 치명이므로 **착수 우선순위는 최상**.

**적용 제외 (기획 충돌 — 2026-08-12 팀 결정, 구현하지 않음)**:

- **미션 팝업 닫기 버튼** — 기능명세서 2.4.3 유효성 검사 "미션 보내기 완료 전에는 팝업이 닫히지 않음" 명시와 정면 충돌. 갇힘 문제는 기존 STT 실패 복구 경로("다시 한번 말해줄래?" 재시도)로 완화.
- **헤더 상시 로그아웃 버튼** — 기능명세서상 로그아웃은 3.1 마이페이지 한정(확인 팝업), 아이 화면은 헤더 없는 몰입 설계. "비로그인 시 로그인 화면으로"는 proxy.ts(T010)에 이미 구현 — 잔여는 루트(/)뿐이며 T066에서 해소.
- **이전 캐릭터 대사 소리 재생**(항목 15 일부) — 기획에 없음 + 타입캐스트 월 3만 자 한도 리스크. 대화 내역은 기능명세서가 요구하는 텍스트 누적 표시만 구현(T076).
- **구글 로그인 버튼**(항목 1) — 기획은 "카카오/구글/네이버 중 1개 이상"으로 카카오 버튼(T044)이 이미 충족. 카카오 동작 불가가 확인될 때만 R-10 대체 플랜(구글) 실행 — 팀 확인 대기.

### 태스크 (쉬운 순)

- [x] T066 \[P\] (파트2) index 라우팅 재정의 — `src/app/page.tsx`의 create-next-app 보일러플레이트 제거, 서버 컴포넌트에서 세션 확인 후 미인증→`/login`, 인증→`/profiles` 리다이렉트 (E2E 항목 31·9 후반. proxy.ts 보호 프리픽스에 `/`가 없어 루트만 뚫려 있음) ✓ 완료(2026-08-12): 보일러플레이트 전체 삭제, `getAuthedUser()`(기존 서버 셸 패턴) 분기 리다이렉트만 남김. 실측: 미인증 `/` 307→`/login`·`/login` 200, tsc 통과

- [x] T067 \[P\] (파트1→파트2 대행) 재구성 발화 활동 안내 문구 — `src/components/retelling.tsx` 상단에 무엇을 하는 활동인지 안내 타이틀 추가(예: "장면 카드와 핵심 단어를 보고, 이야기를 처음부터 끝까지 말해봐요!"), 아이 화면 최소 18px·Cafe24 Ssurround 준수 (E2E 항목 28. 기획에 안내 요소 누락 — 보완, 안내 음성은 TTS 한도 고려 제외) ✓ 완료(2026-08-12): 2.4.4 카드 배열 안내 문구와 동일 스타일(`font-display text-2xl` — Cafe24 Ssurround·24px)로 "장면 카드와 핵심 단어를 보고, 이야기를 처음부터 끝까지 말해보세요!" 추가
...
- [x] T070 \[P\] (공동→파트2 대행) UI 중앙 정렬 점검 — 전 화면 콘텐츠 영역 max-width + 좌우 여백 가운데 정렬 일괄 점검·수정 (E2E 항목 11. 핸드오프 가이드 §2.2 "PC 확장: 콘텐츠 max-width 제한, 가운데 정렬" 명시 위반) ✓ 완료(2026-08-12): 전 화면(로그인·가입·프로필·홈·목록·상세·play·activity·complete·my 5종) 최상위 컨테이너 전수 점검 — **위반 1곳 발견·수정**: 홈 `<main>`에 `items-center` 누락으로 이어하기·추천 섹션(max-w-3xl)이 넓은 화면에서 좌측 쏠림. 나머지는 `mx-auto` 또는 `items-center` 기적용 확인(Header·GNB 풀블리드는 §2.1 고정 원칙대로 유지). **후속(스테이징 스크린샷 제보)**: 2.1 아이 프로필 선택 카드 그리드가 `sm:grid-cols-4` 고정이라 아이 1~2명일 때 빈 열 탓에 카드만 좌측 쏠림 — 컨테이너가 아닌 내용물 쏠림이라 전수 점검에서 누락된 유형. flex-wrap `justify-center`+카드 고정폭(w-44 — 기존 4열 폭 동일)으로 교체, 3명+추가 카드 시 배치 불변

- [x] T071 \[P\] (파트2) 아이 화면 스크롤 제거 — E2E에서 발견된 스크롤 발생 화면 특정 후 뷰포트 내 수납(파트1 소유 화면은 T062에서 점검 완료 — 대화 `dialogue-scene`·학습완료 활동 화면 등 파트2 소유 화면 유력) (E2E 항목 30. 핸드오프 가이드 §2.2 "아이 대상 화면 스크롤 미허용" 명시 위반) ✓ 완료(2026-08-12): **원인 특정 — play·activity `<main>`이 `min-h-dvh`(성장형)라 대화 내역이 누적되면 내부 리스트의 `overflow-y-auto`가 발동하지 않고 페이지 자체가 자라 화면 스크롤 발생.** `h-dvh + overflow-hidden`으로 뷰포트 고정, flex 체인에 `min-h-0` 보강(narration·card-ordering·retelling) — 대화 내역은 이제 리스트 내부 스크롤로만 동작. card-ordering·retelling 섹션에는 `overflow-y-auto` 안전판(극단적으로 짧은 뷰포트에서 버튼 클리핑 방지 — 통상 뷰포트에선 비발동). tsc 통과

- [x] T072 \[P\] (파트2) 대화 화면 재녹음 버튼 — 보내기 전 STT 결과가 마음에 안 들면 재녹음 가능하게(미션 팝업 포함). T054 재구성 화면의 "재클릭 재녹음 무제한" 패턴을 대화 턴 사이클에 이식 — "보내기 클릭 전까지 어떤 발화도 저장되지 않음" 원칙과 합치 (E2E 항목 13. 기획 무충돌 보완 — 기획 보완 기록 필요) ✓ 완료(2026-08-12): 턴 스토어에 `rerecord()` 전이 추가(REVIEW→RECORDING·STT 폐기, REVIEW 외 무시 — TDD 2건, 계 10/10), 대화 화면 마이크를 REVIEW에서도 활성('다시 말하기' 라벨)·클릭 시 rerecord 후 녹음 재시작. 미션 팝업도 동일(REVIEW 재녹음 시 setStt(null) 폐기). SUBMITTED 이후는 여전히 되돌리기 불가(순차 진행형 유지). tsc 통과

- [x] T073 \[P\] (파트2) 캐릭터 프롬프트 보강 — `src/lib/llm/generate.ts` 2건: ① 역할 고정 — 아이가 상대 캐릭터 편을 들어도 자기 캐릭터의 입장·감정을 유지(시아버지가 며느리 편드는 이탈 방지) ② 미션 직전 브릿지 — 장면 목표(방귀 활용)로 전환 시 아이 발화를 받아 자연스럽게 연결(급전환 방지) (E2E 항목 17·22) ✓ 완료(2026-08-12): 시스템 프롬프트에 \[역할 유지\] 섹션 신설(역할 이탈·타 인물 대변 금지, 이견 시에도 자기 입장 유지+질문), \[응답 형식\]에 화제 전환 브릿지 규칙(직전 발화 단어·생각을 받아 연결, 점프 금지) + GUIDED 유도 지시에도 동일 원칙 명시. 프롬프트 조립 테스트 2건 추가(계 13/13). **eval/run.ts는 분석 LLM 회귀라 이번 변경(생성 프롬프트) 비대상** — 실동작 회귀는 T074에서 simulate CLI로 일괄 확인. **후속(2026-08-13, 스테이징 실사용 제보)**: 아이가 며느리 편을 들면 시아버지가 한 턴 만에 "맞아!"로 입장을 뒤집는 과속 동의 + "너는 왜 참았을까?" 주어 오용 + "정진욱," 성 포함 호명 발견 → ① 점진적 설득 규칙(즉각 동의 금지·감정 잔류 문구) ② 질문 주어 명확화(인물 질문은 인물 주어, '너'는 아이 자신에게만) ③ 호칭 givenName+호격(성·쉼표 금지). 테스트 4건 추가(계 19), simulate 양방향(quick-agree 신규·father-side 회귀) 확인 — "그건 맞아, 하지만…"+"며느리는 왜 참았을까?"+"진욱아" 패턴 정착

- [x] T074 (파트2) 미션 강제 노출 폴백 ★데모 치명 (E2E 항목 21) ✓ 완료(2026-08-12): **조사 결과 "2턴 경과 강제 노출"(③)과 "GOAL_MET 시 미션 보류"는 기존 코드가 이미 보장** — 실제 결함은 `mission.ts`의 `missionPhase !== null` 즉시 차단: 노출 턴에 DB가 'exposed'로 기록된 뒤 클라이언트 팝업이 유실되면(새로고침·재진입·응답 유실 — 팝업은 turn 응답으로만 열리는 휘발 상태) **재노출이 영구 차단**되고, GOAL_MET 보류 루프가 MAX_TURNS까지 돈 뒤 미션 없이 장면 종료. 수정: 'completed'만 차단하고 'exposed' 미완료는 `REEXPOSED` 사유로 즉시 재노출(다음 아이 발화 턴에 팝업 복구). 테스트 3건 추가(계 20/20). **simulate 실측 3종**: mission1-stall(③ 2턴 노출→완료→GOAL_MET), mission2-affirmation(1턴 SELF_AFFIRMATION 노출), 신규 dialogue2-father-side — T073 검증 겸용: 아이가 시아버지 편을 들어도 며느리 두둔 단정 대신 질문형 유도로 역할 유지 확인(프롬프트 추가 보강: 장면 목표를 "아이가 이룰 것"으로 명시, 타 인물 사정은 질문으로만, 오프닝 감정 유지 — 시나리오 회귀 자산으로 커밋)

- [x] T075 (파트2) 입력측 부적절 발화 처리 (E2E 항목 24) ✓ 완료(2026-08-12): DB validity enum 변경 없이 구현 — ① 신설 `src/lib/safety.ts` `containsInappropriateLanguage()`(순수 코드 비속어 판정, 부분 문자열 — 오탐 대가는 부드러운 복귀뿐이라 수용, TDD 3건) ② `generate.ts` GenerateContext에 `redirect: 'INAPPROPRIATE'|'OFF_TOPIC'` 추가 — \[주제 복귀\] 지시(따라 말하기·칭찬 금지, 나무람 없이 짧게 받아넘기고 주제 초대 질문, 테스트 3건) ③ /api/turn·simulate가 비속어 판정 또는 분석 validity=OFF_TOPIC일 때 redirect 전달. **차단·저장 배제는 하지 않음**(발화 원문은 리포트 자산, 턴 카운트·GUIDED 전환은 기존 저정보 카운터가 담당). simulate 실측(신규 redirect-inappropriate 시나리오): 비속어 비반복+경계 표현+복귀 질문, 딴 얘기는 아이 관심사를 받아 주제 브릿지 확인. 전체 vitest 143/143·tsc 통과

- [x] T076 (파트2) 대화 내역 리스트 (E2E 항목 15) ✓ 완료(2026-08-12): 내역 리스트 자체는 T037에 이미 있었으나(하단 배치·현재 대사 중복) E2E 요구대로 재배치 — lg+ 화면(태블릿 가로·PC)에서 좌=현재 턴(이미지·캐릭터 카드·상태·STT·버튼, 현행 유지)/우=이전 대화 목록(w-80 aside, "지금까지 나눈 이야기" 타이틀·빈 상태 문구) 2단, 좁은 화면은 세로 스택(목록 max-h-40). **카드에 떠 있는 현재 캐릭터 대사는 목록에서 제외**(findLastIndex — "이전 대화"만 누적). 내부 스크롤(overflow-y)만 허용 — T071 h-dvh 고정과 합치. 이전 대사 소리 재생은 적용 제외(상단 결정). lint 신규 이슈 0(잔여 5건은 기존 T036 패턴), tsc·vitest 143/143 통과. ⚠️ 실기기 시각 확인은 Phase 9 체크포인트(quickstart §4 재리허설)에서

- [x] T077 (파트2) 태블릿 기준(1194×834) 한 화면 수납 감사 — 스테이징 스크린샷 제보("좌우 스크롤") 후속 ✓ 완료(2026-08-12): 로그인 상태 브라우저 실측(1194×834 iframe 하네스, 스테이징·로컬 교차) — **가로 오버플로우는 전 라우트 0건**(제보된 가로 스크롤바는 DevTools 에뮬레이션 캔버스 스크롤바로 확인, /stories 목록도 세로 포함 정상 수납). 세로 초과 4곳 중 아이 여정 2곳 수정: ① 홈 1075→834(헤더 py-3·gap/pb 압축·이어하기 썸네일 96px·추천 카드 aspect-square→16/9·p-2 — 이어하기+추천 3×2+GNB 동시 한 화면) ② 이야기 상세 974→834(이미지 max-h-\[34vh\] 캡·여백 압축). **/my(+30)·/my/badges(+101)는 기획대로 유지 확정(2026-08-12 팀 결정)** — 핸드오프 §2.1·2.2 "보호자 화면 = 세로 스크롤 허용 + Header/GNB 고정" 명시라 기획 정합, 수정하지 않음. play 계열은 T071 h-dvh로 이미 수납 확인. 측정용 테스트 계정·세션은 캐스케이드 정리(잔존 0)

- [x] T078 (파트1) 소셜 로그인 완성 — 카카오 KOE205 복구 + 구글 버튼 추가 (기능명세서 1.1.2, R-10 대체 플랜 실행) ✓ 완료(2026-08-13): **카카오** — 원인은 Supabase GoTrue가 카카오 scope에 `account_email`을 하드코딩(클라이언트 `scopes` 옵션은 append만 — 소스·authorize 실측 확인)하는데 카카오 앱 동의항목의 이메일이 '권한 없음' 상태 → 로그인 직후 KOE205. 코드 우회 불가라 콘솔 해소: 비즈 앱 전환(개인 개발자)+이메일 동의항목 활성(사람 작업). **구글** — Supabase provider·클라이언트 등록은 이미 완비, `login-screen.tsx` 소셜 핸들러 일반화(kakao|google)+G 로고 버튼 신설. **Supabase URL Configuration(Management API)** — `uri_allow_list`가 빈 값+site_url localhost라 배포 서버 복귀가 localhost로 튕기는 상태 발견 → `http://localhost:3000/**`+`https://good-question-*-crewve.vercel.app/**`(해시 배포 URL 전체 커버) 등록, site_url 스테이징으로 변경. ※`good-question.vercel.app`은 타인 소유 앱이라 제외. **후속 진범 2건**: ① 소셜 버튼 글자 실종 제보 → `--color-base` 색 토큰이 폰트 유틸 `text-base`(16px)를 크림색 글자색으로 뒤집는 이름 충돌(카카오 버튼은 T044부터 잠복) — 토큰 제거+`bg-base`→`bg-background` 7파일 치환, globals.css에 재발 방지 주석 ② `color-scheme: only light` 전역 선언(라이트 고정 기획 §5 정합). **검증**: 스테이징 배포 CSS 실측(`.text-base{font-size…}` 복원)+사용자 실로그인 확인(카카오·구글 모두 성공, /profiles 복귀). 배포는 crewve/good-question `vercel deploy --target=staging`(env가 staging 환경 스코프 — Preview는 500). 커밋 bc4d443·b30f7ce·9b5136c

**Checkpoint**: quickstart §4 E2E 시나리오 재리허설 — 특히 미션 노출(대화3·4)·index 진입·프로필 이미지 확인

---

## Phase 10: UI 전면 리뉴얼 — 피그마 「개발 배포용」 대조 (2026-08-13)

- [x] T080 (파트2) 피그마 원본 디코딩 파이프라인 ✓ 완료(2026-08-13): .fig 바이너리 판독 수단을 확보해 R-14 "세부 수치는 피그마에서 직접 추출"을 상시 실행 가능하게 함 — 이전까지는 핸드오프 가이드(원칙 문서) 기준으로 구현해 세부 수치 편차가 생길 수 있었음. `design/GoodQuestion.fig`(ZIP: canvas.fig kiwi + 이미지 116)를 fig-kiwi+kiwi-schema 0.5.0+zstd로 직접 디코딩, 「개발 배포용」 83프레임의 스펙 추출기(`spec.mjs`)·시안 재현 HTML 렌더러 구축(세션 스크래치패드 figx/ — 리포 외부). 로고는 .fig 내장 이미지에서 추출 → `public/goodquestion-logo.png`
- [x] T081 전 화면 리뉴얼 — 5그룹 병렬(워크트리 격리) + 공용 ✓ 완료(2026-08-13, 브랜치 `part2-ui-renewal` 커밋 7개): A 로그인·회원가입·프로필 선택/추가 / B 홈·이야기 목록·상세 / C 도입·전개·대화·미션 팝업(로직 diff IDENTICAL 확인) / D 카드 맞추기·문장만들기·학습완료·배지 / E 내정보·공지·고객센터·이용안내·프로필 관리 / 공용 GNB·로고. **공통 원칙**: 시안 절대 기준, 단 ① 아동 하한선(18px·48px·대비 4.5:1) 미달 시 하한 우선 ② 미구현 기능(네이버 로그인·SMS 인증·필독 배지 등)은 미배치 — 구글 로그인은 이후 T078(파트1) 완성으로 develop 병합 시 복원 ③ 표기 문구는 기능명세서 원문 우선(SSOT) ④ 데이터 정합(배지 3/7권 등) 유지 — 상세 편차는 각 커밋 본문·에이전트 보고 참조. 검증: 통합 tsc 통과·그룹별 eslint 클린. ⚠️ 잔여: 로그인 후 전 화면 브라우저 QA → 확인 후 푸시. 후속 제안(선택): 반복 arbitrary 색상 토큰 승격(#F0E4D3·#FFE8C9·대비 보정 딥 컬러), 홈 헤더 아바타용 avatar_key 조회 추가(팀 확인 필요)
- [ ] T082 (파트1) 네이버 로그인 연동 (기능명세서 1.1.2) — Supabase Auth가 네이버 프로바이더를 미지원하므로 **자체 OAuth 인가 코드 플로우**로 구현(2026-08-15 코드 완료, 네이버 앱 키 발급 대기): `/auth/naver`(시작 — state 쿠키+네이버 인가 페이지 이동) → `/auth/naver/callback`(state 검증 → 토큰 교환 → `openapi.naver.com/v1/nid/me` 프로필 → admin `createUser`(email_confirm, `app_metadata.provider='naver'` — 3.1 로그인 방식 문구 T056 매핑 호환) → admin `generateLink(magiclink)`로 메일 발송 없이 토큰 해시만 뽑아 `verifyOtp`로 세션 쿠키 발급 → `/profiles`). 공용 순수 함수는 `src/lib/auth/naver.ts`(+vitest 6케이스). **같은 이메일 기존 계정은 자동 통합**(카카오·구글의 검증된 이메일 통합과 동일 정책 — 2026-08-15 사용자 확인). 실패 전 갈래는 1.1.2 원문대로 `/login?error=social` 수렴, 키 미설정 배포는 버튼 클릭 시 "준비 중" 안내 폴백(`naverEnabled` 프롭). env: `NAVER_CLIENT_ID`/`NAVER_CLIENT_SECRET`(서버 전용, .env.example 갱신 — Vercel staging·production 환경에도 등록 필요). ⚠️ 잔여: ① 네이버 개발자센터 앱 등록(Callback URL `<origin>/auth/naver/callback` — localhost·프로덕션 고정 도메인만 사전 등록 가능, 스테이징 해시 URL은 매 배포 변경이라 등록 불가) ② 키 수령 후 실로그인 검증
- [ ] T083 (파트1) 단어장 (피그마 「개발 배포용」 2.6 단어장 일체 — 목록/개별단어장 단어1~5/퀴즈1~5·선택·정답·오답·마지막) ✓ 구현 완료(2026-08-15, 브라우저 QA 대기): **⚠️ 기능명세서 충돌 기록** — 기능명세서 2.0/2.2/3.1 GNB "단어장 클릭 시 이동 없음"·IA 4.1 "후순위"였으나 2.6 시안 신설(8/15)로 `/wordbook` 이동 활성. 기능명세서 개정 필요(팀 공유), 시안·전달 자료가 신규 SoT. **구현**: 피그마 REST API 실측(로컬 .fig는 8/10 구본이라 미포함 — FIGMA_TOKEN으로 원격 추출, 렌더 PNG 대조) — ① 목록 `/wordbook`(헤더 69·카드 369×238=이미지 182+제목 56·3열 gap20·그리드 내부 스크롤, 방귀만 활성·나머지 6종 흐림+준비 중은 QA 3 관례 적용 — 시안엔 구분 없음) ② 개별 단어장 `/wordbook/[storyId]`(좌 리스트 h95 선택 시 primary 보더+#FFF0E8 / 우 상세 카드: 일러스트 181²·단어 fs26 primary·뜻 fs24·"이야기 속에 나왔던 말" 배지·인용 강조 칩·소리내어 듣기 남색 그라데이션 버튼 / CTA 단어 게임 하러가기) ③ 퀴즈 `/wordbook/[storyId]/quiz`(헤더 나가기+진행바, 흰 카드: 장면 이미지 350×262·힌트 듣기 칩·빈칸 문제 fs22(□→_ 표시)·보기 h54 클릭 즉시 채점, 팝업: 정답 sage 링/오답 primary X+뜻 상자, 마지막 문제는 "퀴즈 끝내기"→개별 단어장 복귀). **데이터**: DB 없이 fixture SoT(팀 확인 2026-08-15 — 설계서 12. wordbook '아이 저장 단어' 모델과 별개) — `fixtures/wordbook.banggui.json`(단어 5·퀴즈 5, 전달 자료 「0_단어장 파일 전달드립니다._260815」 원문)+`src/lib/wordbook.ts`(+vitest 정합성 4종). **에셋**: 단어 일러스트 5장 design/이미지/단어장/→storage `stories/banggui/wordbook/01~05` 업로드(업로드 스크립트 ASSETS 등록·storage-assets.json 갱신 — 흥부 원본 부재로 신규만 개별 업로드), 퀴즈 장면은 기존 sc_banggui_* 재사용(전개3→06·대화3-2→07의 2컷·대화1→03·대화2→05·전개4→08). **음성**: tts-lines에 kind=wordbook 10건(소리내어 듣기=단어+뜻, 힌트 듣기=정답 채운 문장) 추가·pregenerate 실행(Moonjung 300자, fixed-audio 39/39 검증). GNB 단어장 탭 링크 활성(bottom-nav)+proxy 보호 경로 `/wordbook` 추가. 검증: tsc·eslint·vitest 클린, 미인증 리다이렉트 확인. ⚠️ 잔여: 로그인 브라우저 QA, fixtures/ 변경 파트2 승인(커밋 규칙)
