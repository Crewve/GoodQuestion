...
- [ ] T078 (파트1) 소셜 로그인 완성 — 카카오 KOE205 복구 + 구글 버튼 추가 (기능명세서 1.1.2, R-10 대체 플랜 실행) — 스테이징 제보: 카카오 버튼은 로그인 후 카카오 쪽 에러 페이지(KOE205). 원인 확정(2026-08-13 원격 진단): Supabase GoTrue가 카카오 scope에 `account_email`을 하드코딩(클라이언트 `scopes` 옵션은 append만 — 소스·authorize 실측 확인)하는데 카카오 앱 동의항목의 카카오계정(이메일)이 **권한 없음** 상태 → 콘솔 작업(비즈 앱 전환→이메일 동의항목 활성)이 유일 해법, 코드 우회 불가. 구글은 Supabase provider·클라이언트 등록 완비 확인 — 로그인 화면 버튼만 부재. 작업: ① `login-screen.tsx` 소셜 핸들러 일반화(kakao|google)+구글 버튼 ② 카카오 콘솔 이메일 동의항목 활성(공동 작업) ③ Supabase URL Configuration(배포 origin 리다이렉트 허용 목록) 점검 ④ 배포 서버 E2E 실측
