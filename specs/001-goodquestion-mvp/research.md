...
- **개정(2026-08-15, QA 12)**: `keywords` 스키마를 **`string[]`(장면당 1개) → `string[][]`(장면당 묶음)** 으로 바꾸고, 각 장면에 핵심 단어를 3개씩 실었다. 2.4.5는 카드 아래에 그 묶음을 칩으로 나열한다.
  ...
  - 단어는 각 장면 내레이션에 실제로 나오는 말에서 골랐다(아이가 들은 표현을 그대로 쓸 수 있게): 전개1 `걱정·참기·솔직함` / 전개2 `당황·방귀·갓` / 전개3 `지혜·배나무·마을 사람들` / 전개4 `사과·특별한 힘·도움`.
  - **문서 충돌(CLAUDE.md 개발 기준 §3 기록)**: 기능명세서 2.4.5는 "장면 카드 + 핵심 단어 (4세트)"·유효성 "핵심 단어 4개 포함 여부 검증"으로 **장면당 1개**를 전제하고, 스토리보드(피그마 2.4.5 `단어 활용` 심볼)도 칩 1개다. MVP 요건 원문은 "핵심 단어 3\~4개"로 **총량**만 준다. QA 지시(팀 최신)를 우선해 장면당 3개(총 12개)를 채택했으므로 **명세·시안 표기와 어긋난다**.
...
| 3 | post_activity_config (R-09) | 임시 저작 시드 **반영 완료(2026-08-12, T051)** → **2026-08-15 QA 12로 keywords가 장면당 1개 → 장면당 3개 묶음(`string[][]`)으로 변경, 카드는 4장 유지(seed 재실행 필요)**, 기획 회신 시 데이터만 교체 | 기획(회신 대기) |
