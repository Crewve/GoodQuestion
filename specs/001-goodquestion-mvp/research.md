...
- **개정(2026-08-15, QA 12)**: `keywords` 스키마를 **`string[]`(장면당 1개) → `string[][]`(장면당 묶음)** 으로 바꾸고, 각 장면에 핵심 단어를 2\~4개씩 실었다. 2.4.5는 카드 아래에 그 묶음을 칩으로 나열한다.
  - **QA 12 원문 해석 정정**: "이야기 장면 완성 부분에서 카드 몇가지 더 추가"를 처음에는 *장면 카드 수 추가*로 읽고 카드를 4→5장(도입 추가)으로 늘렸으나, **기획 의도는 "카드 아래 장면별 핵심 단어 추가"** 임이 확인돼(2026-08-15 팀 확인) 카드는 **4장 원상 복구**하고 키워드만 확장했다.
  - 단어 목록은 **기획 지정본**(2026-08-15 전달, 장면당 2\~4개): 전개1 `걱정·고민·불안` / 전개2 `당황·놀람·실수·부끄러움` / 전개3 `방법·지혜` / 전개4 `미안함·반성·사과·이해`. 장면의 감정·해결 과정을 짚는 말로 구성돼 있다. 전개3은 처음 `고민`이 함께 있었으나 장면 내용(높은 배나무 앞에서 방법을 찾는 대목)과 겹치지 않아 뺐다 — 남은 두 단어만 쓴다(2026-08-15 팀 확인).
  - **문서 충돌(CLAUDE.md 개발 기준 §3 기록)**: 기능명세서 2.4.5는 "장면 카드 + 핵심 단어 (4세트)"·유효성 "핵심 단어 4개 포함 여부 검증"으로 **장면당 1개**를 전제하고, 스토리보드(피그마 2.4.5 `단어 활용` 심볼)도 칩 1개다. MVP 요건 원문은 "핵심 단어 3\~4개"로 **총량**만 준다. QA 지시(팀 최신)를 우선해 장면당 2~4개(총 13개)를 채택했으므로 **명세·시안 표기와 어긋난다**.
  - 2.4.4 슬롯 수·2.4.5 열 수는 `cards.length` 파생으로 유지(하드코딩 4 제거) — 콘텐츠 교체만으로 장면 수가 바뀌어도 화면이 따라간다.
  - `parsePostActivityConfig`는 옛 형태(`string[]`)도 `[[단어], …]`로 감싸 받는다 — develop 푸시가 곧 스테이징 배포라 **코드가 시드보다 먼저 나가는 구간**에서 화면이 죽지 않게 하기 위함.
  - **후속 필요**: ① 기능명세서 2.4.5의 "4세트"·"핵심 단어 4개 포함 여부" 표기 개정 ② 스토리보드 2.4.5에 장면당 칩 복수 반영 ③ 단어 목록 기획 확정본 회신 시 fixtures 교체. 데이터 변경이므로 **`scripts/seed.ts` 재실행 없이는 운영 DB에 반영되지 않는다**(실사용 SoT는 `stories.post_activity_config`).
...
## R-19. 대화 화면 QA 반영 — 자동 전송·가이드라인 노출 (2026-08-15, 배치 B)

- **충돌 기록**: QA 8번(보내기 버튼 없이 바로 전송)은 기능명세서 2.4.3의 "REVIEW(수정 불가 미리보기) → **보내기 클릭 시에만** /api/turn 호출·저장·분석" 규정과 상충한다. QA 27번(가이드라인은 첫 채팅에만)도 상태 카드 하단 문구를 매 턴 공통 표기로 둔 시안과 어긋난다.
- **Decision**: QA 결정을 채택 — ① 전사 성공 즉시 자동 전송(`REVIEW`는 상태머신에 남되 스스로 지나가는 통과 단계, 별도 미리보기 카드 제거), ② 격려 문구(가이드라인)는 **대화 장면의 첫 채팅에만** 노출(아이 발화가 하나라도 쌓이면 감춤). "전송 시점에만 저장·분석"이라는 계약 불변 조건 자체는 유지된다 — 전송 트리거만 사람 클릭에서 전사 성공으로 바뀐 것.
- **부수 효과**: T072(E2E 항목 13 — REVIEW에서 재녹음 후 보내기)는 더 이상 정상 경로가 아니다. `rerecord()` 전이는 남기되, 마이크 재클릭 재녹음은 게이트/STT 실패 복구용으로만 도달한다.
- **함께 반영**: QA 28번(다시 듣기 ↔ 녹음 상호 배타 — 캐릭터 음성이 마이크에 섞이는 것 방지), QA 29번(녹음 중 입력 레벨 표시 강화 — `src/lib/mic-level.ts` 순수 함수로 RMS→표시 강도 변환).
- **미결**: 미션 팝업(`mission-popup.tsx`)의 보내기 버튼은 배치 C 소유 파일이라 이번 범위에서 제외 — QA 8번을 미션 팝업에도 적용할지 기획 확인 필요.
- **개정 대상**: 기능명세서 2.4.3(전송 트리거·가이드라인 노출), `spec.md` FR-007("보내기 클릭 시에만"), `data-model.md` §5 REVIEW 설명("텍스트 표시·보내기 활성"), `test-checklist.md` B-2c·D-3("보내기 후 캐릭터 응답").
...
| 3 | post_activity_config (R-09) | 임시 저작 시드 **반영 완료(2026-08-12, T051)** → **2026-08-15 QA 12로 keywords가 장면당 1개 → 장면당 2~4개 묶음(`string[][]`)으로 변경, 카드는 4장 유지(seed 재실행 필요)**, 기획 회신 시 데이터만 교체 | 기획(회신 대기) |
